...
import { cancelWorkOrder, createWorkOrder, listWorkOrders, type WorkOrder, type WorkOrderStatus } from './api';

const STATUS_LABELS: Record<WorkOrderStatus, string> = { open: 'Open', completed: 'Completed', cancelled: 'Cancelled' };

export default function WorkOrdersPage() {
  const [orders, setOrders] = useState<WorkOrder[]>([]);
  const [form, setForm] = useState({ customerName: '', locationName: '', address: '', tankLocation: '' });
  ...
  const refresh = useCallback(async () => {
    try { setOrders(await listWorkOrders()); setError(''); }
    catch (value) { setError(value instanceof Error ? value.message : 'Work orders are unavailable.'); }
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setForm({ customerName: '', locationName: '', address: '', tankLocation: '' });
      await refresh();
  ...
  async function confirmCancellation(order: WorkOrder) {
    ...
    try { await cancelWorkOrder(order, reason.trim()); setPendingId(null); setReason(''); await refresh(); }
    ...
    <section className="fleet-page" aria-labelledby="work-orders-heading">
      ...
      <div className="detail-panel">
        ...
        <form className="form-grid two-column" onSubmit={submit}>
          ...
          <div className="form-actions span-two"><button className="button button-primary" type="submit" disabled={busy}><ClipboardPlus size={15} /> Create work order</button></div>
        </form>
      </div>
      ...
      <div className="fleet-table-shell">
        <table className="staff-table work-orders-table">
          ...
          <tbody>
            {orders.length === 0 && <tr><td className="staff-empty" colSpan={6}>No work orders yet.</td></tr>}
